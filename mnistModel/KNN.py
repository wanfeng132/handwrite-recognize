...
def KNN(test_data, dataSet, labels, k):
    ...
    # 计算欧氏距离开始
    # #tile函数在行上重复dataSetSizec次，在列上重复1次
...
# KNN分类
# test_KNN(）函数中调用了读取数据集的函数，并调用分类函数对数据集进行分类，最后对分类情况进行计算
def test_KNN():
    # 文件获取
    # mnist数据集
    ...
    testRatio = 0.005      # 取数据集的前0.1为测试数据,这个参数比重可以改变,可看出训练集大小对结果的影响
    train_row = train_x.shape[0]    # 数据集的行数，即数据集的总的样本数
    test_row=test_x.shape[0]
    testNum = int(test_row * testRatio)
    errorCount = 0  # 判断错误的个数
    for i in range(testNum):
        # 调用KNN 最后一个参数为K的值 K=1时为最邻近分类
        result = KNN(test_x[i], train_x, train_y, 1)
        # 返回分类结果与真实结果
        print("The classifier came back with:"+ str(result) + "," + " The real answer is:" + str(test_y[i])+"\n")
        if result != test_y[i]:
